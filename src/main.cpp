        ...
        if (key.key == 'B') {
            ...
            if (keyValue != "") {
                ...
                if (keyValue == "scan") {
                    ...
                    clearScreen();
                    drawBmp("/games/bmp2.bmp", 0, 0, tft);
                } else if (keyValue == "cls" || keyValue == "clear") {
                    clearScreen();
                }
                else if (keyValue == "ls") {
                    clearScreen();
                    tft->println();
                    listDir(SPIFFS, currentDirectory.c_str(), 0, tft);
                    keyValue.clear();
                    printCursor();
                }  else if (keyValue[0] == 'c' && keyValue[1] == 'd') {
                    String nextDir = currentDirectory + keyValue.substring(3, keyValue.length());
                    // tft->println(nextDir);
                    if (chDir(SPIFFS, nextDir.c_str())) {
